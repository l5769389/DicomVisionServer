...
def test_series_volume_cache_stores_integral_volume_as_contiguous_int16() -> None:
...
def test_series_volume_cache_evicts_lru_entries_and_reports_callback() -> None:
    evicted: list[str] = []
    cache = SeriesVolumeCache(max_bytes=16, on_evict=lambda series_id, volume: evicted.append(series_id))
    ...
    assert int(first.nbytes) == 16


def test_series_volume_cache_preserves_fractional_values_as_float32() -> None:
    cache = SeriesVolumeCache(max_bytes=1024)

    stored = cache.store("series-fractional", np.array([[[0.25, 1.5]]], dtype=np.float64))

    assert stored.dtype == np.float32
    np.testing.assert_allclose(stored, np.array([[[0.25, 1.5]]], dtype=np.float32))
