...
def _patch_pet_render_dependencies(monkeypatch, service: ViewerService, series: SeriesRecord, volume: np.ndarray) -> None:
...
def test_pet_config_window_updates_range() -> None:
    service = ViewerService()
    view = ViewRecord(view_id="pet-view", series_id="pet", view_type="PET", width=128, height=96)

    changed = service._handle_pet_config(
        view,
        ViewOperationRequest(viewId=view.view_id, opType="petConfig", petWindowMin=1.0, petWindowMax=9.0),
    )
