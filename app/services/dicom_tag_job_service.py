...
class DicomTagModifyJobService:
    ...
    def _get_job_locked(self, job_id: str, workspace_id: str | None = None) -> DicomTagModifyJob | None:
        job = self._jobs.get(job_id)
        if job is None:
            return None
    ...
    def _status_url(cls, job_id: str, workspace_id: str) -> str:
        return f"/api/v1/dicom/modifyTag/jobs/{job_id}{cls._workspace_query(workspace_id)}"

    @classmethod
    def _artifact_url(cls, job_id: str, workspace_id: str) -> str:
        return f"/api/v1/dicom/modifyTag/jobs/{job_id}/artifact{cls._workspace_query(workspace_id)}"
