...
class PacsWadoDownloadJobService:
    ...
    def create_job(self, payload: PacsWadoSeriesDownloadRequest) -> PacsWadoSeriesDownloadJobStatusResponse:
        self.cleanup_cache()
        job_id = uuid4().hex
        job = PacsWadoDownloadJob(job_id=job_id, status="pending", created_at=utc_now())
        with self._lock:
    ...
    def get_status(self, job_id: str) -> PacsWadoSeriesDownloadJobStatusResponse:
        with self._lock:
    ...
    def _status_url(job_id: str) -> str:
        return f"/api/v1/pacs/dicomweb/downloadSeries/jobs/{job_id}"
