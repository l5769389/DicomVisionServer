...
from app.schemas.pacs import PacsDimseSeriesDownloadJobStatusResponse, PacsDimseSeriesDownloadRequest
from app.services.dicom_job_utils import format_datetime, normalize_progress_counts, resolve_progress_percent, utc_now
from app.services.pacs_dimse_service import PacsDimseError, PacsDimseService, pacs_dimse_service
from app.services.series_registry import series_registry


PacsDimseDownloadJobState = Literal["pending", "running", "succeeded", "failed", "cancelled"]
logger = get_logger(__name__)


@dataclass
class PacsDimseDownloadJob:
    job_id: str
    status: PacsDimseDownloadJobState
    created_at: datetime
...
class PacsDimseDownloadJobService:
    ...
    def __init__(
        self,
        *,
        dimse_service: PacsDimseService | None = None,
        ...
        self._executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="pacs-dimse-download")
        self._jobs: dict[str, PacsDimseDownloadJob] = {}
        self._lock = threading.Lock()
        self._dimse_service = dimse_service or pacs_dimse_service
    ...
    def create_job(self, payload: PacsDimseSeriesDownloadRequest) -> PacsDimseSeriesDownloadJobStatusResponse:
        self.cleanup_cache()
        job_id = f"dimse-{uuid4().hex}"
        job = PacsDimseDownloadJob(job_id=job_id, status="pending", created_at=utc_now())
        with self._lock:
    ...
    def get_status(self, job_id: str) -> PacsDimseSeriesDownloadJobStatusResponse:
        with self._lock:
    ...
    def cancel_job(self, job_id: str) -> PacsDimseSeriesDownloadJobStatusResponse:
    ...
    def _run_job(self, job_id: str, payload: PacsDimseSeriesDownloadRequest) -> None:
        ...
        try:
            ...
            received_count = self._dimse_service.retrieve_series(
                payload.profile,
                study_instance_uid=payload.study_instance_uid,
                series_instance_uid=payload.series_instance_uid,
                output_dir=download_dir,
                progress_callback=lambda processed, total: self._update_progress(job_id, processed, total),
                should_cancel=lambda: self._is_cancel_requested(job_id),
            )
            self._update_progress(job_id, received_count, received_count)
    ...
    def _should_stop_cancelled_job(self, job_id: str) -> bool:
        should_stop = self._is_cancel_requested(job_id)
        if should_stop:
            self._mark_cancelled(job_id)
        return should_stop

    def _to_response(self, job: PacsDimseDownloadJob) -> PacsDimseSeriesDownloadJobStatusResponse:
    ...
    def _status_url(job_id: str) -> str:
        return f"/api/v1/pacs/dimse/downloadSeries/jobs/{job_id}"


pacs_dimse_download_job_service = PacsDimseDownloadJobService(start_cleanup_worker=True)
