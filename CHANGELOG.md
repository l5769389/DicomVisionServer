...
- 新增结构化体数据兼容性结果，覆盖切片数量、空间坐标、方向、PixelSpacing、重复位置、不规则层距和多帧限制。
- 4D 能力检查逐相位复用 MPR 几何校验；任一相位不兼容时返回分组后的明确原因。
- MTF 点源分析增加径向及 W/H 方向 MTF50、MTF10、FWHM、Nyquist 和质量警告。

### 改进

- MTF 在完整矩形 ROI 上进行稳健背景拟合、点源极性识别、半高交点插值和零填充频谱计算。
- 对过小 ROI 自动扩展，对零频响应不稳定和低信噪比场景提供带警告的降级结果；不可恢复条件返回结构化 `422`。
- Stack/PET 调窗预览使用全尺寸后端渲染，并统一初始适配、resize 和 reset 的物理比例计算。
- MPR、3D、4D 创建前执行统一兼容性检查，避免留下无法渲染的 View 或 ViewGroup。

### 修复

- 修复 FWHM 使用像素计数近似、MTF 阈值未穿越时返回频率末端值，以及曲线被错误强制裁剪的问题。
- 修复 DICOM `[row, column]` PixelSpacing 在 W/H FWHM 和方向 MTF 中的映射问题。
- 修复非正方形 PixelSpacing 影像在初始显示、视口调整和 reset 后比例或缩放不一致的问题。
- 修复 4D 仅识别时间相位、但未验证各相位能否构建 MPR 的问题。

### 测试

- 增加高斯点源解析真值、方向性分辨率、边缘裁切、低信噪比、Nyquist 和结构化错误测试。
- 增加体数据几何兼容性、4D manifest、View 生命周期和 Stack 物理 contain-fit 回归测试。
