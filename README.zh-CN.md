...
DicomVision Server 是 DicomVision 的后端服务，提供 DICOM 序列发现、缩略图与标签读取、DICOM Tag 修改、DICOM 脱敏导出、Stack 渲染、MPR/斜切 MPR 重建、4D 时相预览与播放协同、VTK 3D 体渲染、测量结果计算、MTF/FWHM 分析、水模 QA、图像导出，以及通过 Socket.IO 向前端实时推送图像帧和交互结果的能力。

## v1.1.0 更新内容

- 新增异步 DICOM Tag 修改任务，支持状态轮询、artifact 下载和逐实例处理进度。
- 新增 DICOM 脱敏服务与异步脱敏导出任务，支持整个 series 生成脱敏副本。
- DICOM artifact 任务响应新增 `processedCount`、`totalCount`、`progressPercent` 等进度字段。
- 优化 DICOM artifact 下载响应头，支持 Unicode 文件名，并统一桌面端与 Web 端保存流程。
- 扩展 Tag 修改、脱敏导出、单文件加载、artifact 响应头和任务进度相关测试。
